(() => {
  ...
  function extractExecutableSnippetFromCSS(css) {
    ...
    while ((match = pattern.exec(css)) !== null) {
      ...
      if (char === "{") {
        ...
        if (stack.length === 1) {
          snippets.push(...extractExecutableSnippetFromString(css.slice(stack[0], match.index), ["[", "]"], stack[0]));
        }
        stack.pop();
      }
    }
 ...
 ${value}`);
    ...
    if (nodeType === 1) {
      ...
      [...attributes].forEach((attr, i) => {
        if (dirPattern.test(attr.name)) {
          ...
          track = new Track(name, value, 2 /* directive */);
          track.prop = prop;
          track.handler = dir;
          if (name === "if") {
  ...
  var _NodeTrack = class {
    ...
    get $context() {
      return (this.anchor === this.node ? $.get(this.node).$context : $.get(Array.isArray(this.anchor) ? this.anchor[0] : this.anchor)?.$context) || {};
    }
    get compiled() {
    ...
    updateNode(force = false) {
      if (this.node.nodeType === 3) {
        ...
        if (force && this.node.nodeName.includes("-") && typeof this.node.forceUpdate == "function") {
    ...
    _updateNodeProperty(track) {
      const newValue = resolveExecutables(track.value, this.component, this.$context, track.executables);
    ...
    _updateNodeAttribute(track) {
      let newValue = resolveExecutables(track.value, this.component, this.$context, track.executables);
      if (isPrimitive(newValue)) {
    ...
    _updateNodeDirective(track) {
      if (track.handler) {
        ...
        let value = resolveExecutables(val, this.component, this.$context, extractExecutableSnippetFromString(val));
        if (typeof value === "string") {
      ...
      try {
        ...
        if (parsed) {
          ...
          if (stylesheet && mode !== "none" || !getLinkAndStyleTagsFromHead(tagName).length) {
            style = typeof stylesheet === "object" ? `<style class="${tagName}">${JSONToCSS(stylesheet)}</style>` : getStyleString(stylesheet, tagName.toLowerCase(), hasShadowRoot);
          }
  ...
  // src/core/context-provider-component.ts
